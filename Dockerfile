...
RUN install-php-extensions pdo_mysql gd intl zip opcache ctype curl dom fileinfo filter ftp hash iconv json libxml mbstring openssl session simplexml soap spl zlib \
  && apt update && apt install -y git patch unzip default-mysql-client \
...
RUN COMPOSER_ALLOW_SUPERUSER=1 composer install --optimize-autoloader --prefer-dist --no-interaction --no-progress --no-dev
